...
How the **LanguageTags** library is laid out and what its public surface promises. [`AGENTS.md`](./AGENTS.md) is the agent entry point, [`GOVERNANCE.md`](./GOVERNANCE.md) holds the cross-cutting rules, [`CODESTYLE.md`](./CODESTYLE.md) the code style, and [`OPERATIONS.md`](./OPERATIONS.md) how the repo is run.
...
- **`LanguageTags`** ([`LanguageTags/LanguageTags.csproj`](./LanguageTags/LanguageTags.csproj)): the library, and the only packable project. Target framework .NET 10.0, AOT compatible (`<IsAotCompatible>true</IsAotCompatible>`). Its source is the publisher's shipped input, so a change here republishes the package.
- **`LanguageTagsCreate`** ([`LanguageTagsCreate/LanguageTagsCreate.csproj`](./LanguageTagsCreate/LanguageTagsCreate.csproj)): the CLI codegen tool. It downloads four registries, ISO 639-2 from the Library of Congress, ISO 639-3 from SIL, RFC 5646 / BCP 47 from IANA, and UN M.49 from the Unicode CLDR supplemental data, converts each to JSON, and generates the C# data files. Never packaged, and never referenced by the library.
- **`LanguageTagsTests`** ([`LanguageTagsTests/LanguageTagsTests.csproj`](./LanguageTagsTests/LanguageTagsTests.csproj)): the xUnit v3 suite, asserting through AwesomeAssertions and running on native Microsoft.Testing.Platform.

[`LanguageData/`](./LanguageData/) holds the embedded registry data the codegen tool refreshes. Its bytes are the upstream registries' own, so [`.gitattributes`](./.gitattributes) and [`.editorconfig`](./.editorconfig) both exempt it from line-ending and whitespace normalization, and a change there is generated rather than written by hand.
...
- **A tag is immutable once constructed.** `LanguageTag`'s properties have internal setters and its `Variants` and `Extensions` collections are `ImmutableArray<T>`, so a consumer cannot alter a tag it holds, and `Normalize()` returns a new copy rather than mutating in place. `LanguageLookup.Overrides` is the deliberate exception, a mutable `IList<T>` because its whole purpose is to let a consumer add its own IETF-to-ISO mappings.
