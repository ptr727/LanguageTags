...
# in BOTH stores: codegen and the publisher read them from Actions, but the merge-bot reads them from the
...
check_app() {
  # Best-effort: confirm a GitHub App installation backs the merge/codegen automation. A precise check
  ...
  if gh api "repos/$REPO/installation" >/dev/null 2>&1; then
    ...
    note "could not confirm App installation via this token (verify the codegen App is installed)"
  fi
}
